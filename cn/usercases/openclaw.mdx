...
description: "将 AllToken 作为 OpenClaw 的自定义 OpenAI 兼容提供方接入。"
---
...
将 AllToken 配置为 OpenClaw 的 OpenAI 兼容提供方后，你可以通过统一网关管理模型访问、API Key 和路由策略。
...
请确认 `--custom-base-url` 使用的是 `https://alltoken.co/v1`，并确认模型名已在 AllToken 中开通。

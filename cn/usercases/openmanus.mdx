---
title: "OpenManus"
description: "将 AllTokens 接入 OpenManus，配置支持 function calling 的模型并运行智能体任务。"
---
...
在 `config` 目录里找到 `config.toml`，把模型服务改成 AllTokens。
...
接入 AllTokens 后，你可以把模型入口统一到一个网关，再按任务选择合适的通用模型或视觉模型。
